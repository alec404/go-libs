...
func Test_Trans(t *testing.T) {
	...
	expected := time.Now().Round(time.Millisecond)
	tm := Time(expected)
	tmVal := TimeValue(tm)
	assert.Equal(t, expected, tmVal)

	nilValue := TimeValue(nil).Round(time.Millisecond)
	assert.Equal(t, time.Now().Round(time.Millisecond), nilValue)
}
